...
from .model import LSTM, Bert, LSTMATTN, LastQuery, Saint, GPT, XLMRoberta
...
def run(args, train_data, valid_data):
    ...
    metric = SphereProduct(args).to(args.device) if args.loss_type == 'arcface' else None
    ...
    for epoch in range(args.n_epochs):
        ...
        ### TRAIN
        train_auc, train_acc, train_loss = train(train_loader, model, optimizer, args, metric)
        
        ### VALID
        auc, acc, total_preds , _ = validate(valid_loader, model, args)

        ### TODO: model save or early stopping
        wandb.log({"epoch": epoch, "train_loss": train_loss, "train_auc": train_auc, "train_acc":train_acc,
                    "valid_auc":auc, "valid_acc":acc, 'best_auc': max(best_auc,auc)})
        if auc > best_auc:
            best_auc = auc
...
def train(train_loader, model, optimizer, args, metric):
    ...
    if args.loss_type == 'roc_star' or args.loss_type == 'both':
...
def get_model(args):
    ...
    if args.model == 'gpt': model = GPT(args)
    if args.model == 'xlm': model = XLMRoberta(args)
...
def compute_loss(preds, targets, args):
    ...
    if args.loss_type == 'bce' or ((args.loss_type == 'both' or args.loss_type == 'roc_star') and args.epoch == 0):
        loss = loss[:,-1]
