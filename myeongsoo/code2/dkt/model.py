...
class LSTM(nn.Module):
    def __init__(self, args):
        ...
        self.embedding_cont = nn.Sequential(
            nn.Linear(self.args.n_cont, self.hidden_dim), nn.LayerNorm(self.hidden_dim)
        )
...
class LSTMATTN(nn.Module):
    def __init__(self, args):
        ...
        # interaction은 현재 correct로 구성되어있다. correct(1, 2) + padding(0)
        self.embedding_interaction = nn.Embedding(3, self.hidden_dim // 3)
        self.embedding_problem_interaction = nn.Embedding(
            13 * 2 + 1, self.hidden_dim // 3
        )
        self.embedding_test = nn.Embedding(self.args.n_test + 1, self.hidden_dim // 3)
        self.embedding_question = nn.Embedding(
            self.args.n_questions + 1, self.hidden_dim // 3
        ...
        self.cate_proj = nn.Sequential(
            nn.Linear(
                (self.hidden_dim // 3) * (len(self.args.cate_col) + 1), self.hidden_dim
            ),
            nn.LayerNorm(self.hidden_dim),
        )
    ...
    def forward(self, input):
        ...
        if "problem_interaction" in input:
            embed_problem_interaction = self.embedding_problem_interaction(
                input["problem_interaction"]
            )
        ...
        if self.args.n_cont > 0:
            cont = torch.cat([input[c].unsqueeze(2) for c in self.args.cont_col], 2)
            cont = self.bn_cont(cont.view(-1, cont.size(-1))).view(
                batch_size, -1, cont.size(-1)
            )
            embed_cont = self.embedding_cont(cont)
...
class Bert(nn.Module):
    def __init__(self, args):
        ...
        self.embedding_question = nn.Embedding(
            self.args.n_questions + 1, self.hidden_dim // 4
        )
        ...
        # embedding combination projection
        self.cate_proj = nn.Sequential(
            nn.Linear((self.hidden_dim // 4) * 5, self.hidden_dim),
            nn.LayerNorm(self.hidden_dim),
        )
        self.embedding_cont = nn.Sequential(
            nn.Linear(self.args.n_cont, self.hidden_dim), nn.LayerNorm(self.hidden_dim)
    ...
    def forward(self, input):
        ...
        embed_cate = torch.cat(
            [embed_interaction, embed_test, embed_question, embed_tag, embed_grade], 2
        )
        embed_cate = self.cate_proj(embed_cate)
...
class Saint(nn.Module):
    def __init__(self, args):
        ...
        self.enc_comb_proj = nn.Linear(
            (self.hidden_dim // 3) * 3, self.hidden_dim
        )  # Layernorm 추가해야 되나?
        ...
        self.cont_bn = nn.BatchNorm1d(self.args.n_cont)
        self.cont_proj = nn.Sequential(
            nn.Linear(self.args.n_cont, self.hidden_dim),
            nn.LayerNorm(self.hidden_dim),
        )
        self.cate_proj = nn.Linear((self.hidden_dim) * 4, self.hidden_dim)
    ...
    def forward(self, input):
        ...
        if "problem_interaction" in input:
            ...
            be_concat.append(embed_problem_interaction)
            batch_size = input["problem_interaction"].size(0)
            seq_len = input["problem_interaction"].size(1)
        ...
        embed_enc = self.enc_comb_proj(embed_cate)
        ...
        # concat categorical and continuous
