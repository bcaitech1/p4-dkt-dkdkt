...
def get_criterion(pred, target, args): 
    ...
    if args.loss_type == 'arcface' :
        arcloss = AngularPenaltyLoss(args) 
        l = arcloss(pred,target[:,-1].contiguous()) 
    elif args.loss_type == 'both' and args.epoch > 0:
    ...
    # if args.loss_type == 'bce':
        l = loss(pred, target) # + (0-pred)**2 + (1-pred) **2
...
class AngularPenaltyLoss(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.s, self.m = 30.0,0.4 # 64.0, 0.5 // 64.0,1.35 // 30.0,0.4
        self.in_features = args.hidden_dim
        self.out_features = 2
        self.fc = nn.Linear(self.in_features, self.out_features, bias = False) # 없이도 써보기!
        self.eps = 1e-7
    def forward(self,x,labels):
        '''input_size = (N,in_feature)''' # x : batch * max_length * hidden_dim // labels : batch * max_length
        for W in self.fc.parameters():
            W = F.normalize(W, p=2, dim=1)
        x = F.normalize(x, p=2, dim=1)
        wf = self.fc(x) # batch * 2 / 64 * 500 * 2 
        
        numerator = self.s * (torch.diagonal(wf.transpose(0, 1)[labels]) - self.m)
        excl = torch.Tensor([wf[abs(1-y)] for i, y in enumerate(labels)])
